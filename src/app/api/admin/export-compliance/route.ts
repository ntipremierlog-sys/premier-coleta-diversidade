...
import { generateComplianceExcel, AggregatedDiversityData } from "@/lib/excel-generator";
  ...
  try {
    ...
    if (!session.isAuthenticated) {
      return NextResponse.json(
        { error: "Não autorizado. Faça login para exportar o relatório." },
        { status: 401 }
      );
    }
    ...
    const excelBuffer = await generateComplianceExcel(
      aggregated,
      submissions as any,
      unidadeParam,
      competenciaParam
    ...
    await logAccessAction({
      ...
      acao: "export_xlsx",
      detalhe: `Exportação Relatório Institucional de Conformidade .xlsx (Unidade: ${unidadeParam}, Competência: ${competenciaParam}).`,
    });
    ...
    const filename = `Relatorio_Institucional_Conformidade_${unidadeSlug}_${competenciaSlug}.xlsx`;
    ...
    console.error("Erro na exportação Relatório Institucional Excel:", error);
    return NextResponse.json(
      { error: "Erro ao gerar Relatório Institucional Excel." },
      { status: 500 }
    );
  }
}
